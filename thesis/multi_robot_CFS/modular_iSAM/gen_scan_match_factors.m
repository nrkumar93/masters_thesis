function scan_match_factor = gen_scan_match_factors(mode ,keyframe_rate, robot_id, var_1, var_2, range_1, range_2, init_1, init_2, time_1, time_2)
...
global scan_matching_covariance key_offset;
    ...
    for i = 1:length(var_1)
        ...
        if ~isempty(scan_match_R) && ~isempty(scan_match_T)
            ...
            if disparity < (0.01 * keyframe_rate) || norm(scan_match_T) < (keyframe_rate * 0.1)
                offset_var_1 = robot_id * key_offset(robot_id) + var_1(i);
                offset_var_2 = robot_id * key_offset(robot_id) + var_2;
                
                scan_match_factor = [scan_match_factor, BetweenFactorPose2(offset_var_1, offset_var_2, ...
